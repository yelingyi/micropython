...
also have to pass the architecture you are running native modules
("--arch rv32imc" for the `VIRT_RV32` board).
...
- `RUN_NATMODTESTS_ARGS`: pass in extra flags for `run-natmodtests.py` when invoked
  via `make test_natmod`.
